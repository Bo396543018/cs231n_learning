...
def rnn_step_backward(dnext_h, cache):
  ...
  dx = (dnext_h * dtanh).dot(Wx.T) # N * H * H * D
  dprev_h = (dnext_h * dtanh).dot(Wh.T) # N * H * H * H
...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  for t in range(T):
    ...
    cache[t] = x[:, t, :], prev_h, Wx, Wh, b, next_h
    prev_h = next_h
    h[:, t, :] = next_h
...
def rnn_backward(dh, cache):
  ...
  for t in range(T):
    ...
    dx[:, t, :], dprev_h, dWxt, dWht, dbt = rnn_step_backward(dnext_h, cache[t])
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  #############################################################################
  for i in range(T):
    next_h, next_c, cache[i] = lstm_step_forward(x[:, i, :], h0, prev_c, Wx, Wh, b)
    h0 = next_h
    prev_c = next_c
    h[:, i, :] = next_h
  ##############################################################################
