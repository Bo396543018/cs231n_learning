...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = x, norm_data, gamma, beta, var, mean, eps
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  elif mode == 'test':
...
def batchnorm_backward(dout, cache):
  ...
  x, norm_data, gamma, beta, var, mean, eps = cache
  N = x.shape[0]
  #############################################################################
...
def batchnorm_backward_alt(dout, cache):
  ...
  dgamma = np.sum(norm_data * dout, axis=0)
  dbeta = np.sum(dout, axis=0)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  if pad:
    for i in xrange(N):
      for j in xrange(C):
        xpad[i][j] = np.lib.pad(x[i][j], (pad, pad), 'constant', constant_values=0)
  #############################################################################
  ...
  #for i in xrange(N):
   # for j in xrange(F):
     # for k in xrange(C):
      #  for l in xrange(Wn):
       #   for m in xrange(Hn):
        #    out[i, j, l, m] += np.sum(xpad[i][k][l * pad: l * pad + HH,\
         #                          m * pad: m * pad + HH] * w[j][k]) + b[j]
    for i in range(H_out):
      for j in range(W_out):
        x_pad_masked = xpad[:, :, i * stride:i * stride + HH, j * stride:j * stride + WW]
        for k in range(F):
